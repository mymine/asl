...
restore_backup() {
    ...
    read -p "选择要还原的备份 (1-$backup_count): " restore_choice
    
    if [[ "$restore_choice" -ge 1 && "$restore_choice" -le $backup_count ]]; then
        local restore_dir="${available_backups[$((restore_choice-1))]}"
...
download_system_menu() {
    while true; do
        ...
        read -p "请选择要下载的系统: " system_choice
        ...
        read -p "选择版本 (留空为最新版本): " version_choice
    ...
    while true; do
        ...
        case $main_choice in
            ...
            3)
                ...
                read -p "请选择镜像源 (default/tuna/ustc/sjtu): " mirror
